...
module.exports = (app) => {
  app.get("/tools/removebg", checkApiKeyAndLimit, async (req, res) => {
    const { url } = req.query;
    ...
    try {
      ...
      if (flow.data?.result) return res.json({ status: true, result: flow.data });
    } catch (_) {}

    try {
      const neko = await axios.get(`https://api.nekorinn.my.id/tools/removebg?imageUrl=${encodeURIComponent(url)}`);
      if (neko.data?.result) return res.json({ status: true, result: neko.data });
    } catch (_) {}
