...
class Rextester {
  ...
  async runCode({ code = 'print("Hello, world!")', lang = "python3" }) {
    ...
    try {
      ...
      return { error: error.response?.data || "Error executing code" };
    }
  }
}

module.exports = (app) => {
  const rex = new Rextester();

  app.get("/tools/rextester", checkApiKeyAndLimit, async (req, res) => {
    ...
    try {
      const result = await rex.runCode({ code, lang });
      res.json({
        status: true,
        result
      });
