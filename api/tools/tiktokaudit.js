...
module.exports = (app) => {
  app.all("/tools/translate", checkApiKeyAndLimit, async (req, res) => {
    const params = req.method === "POST" ? req.body : req.query;
    const { text, target = "ID", source = "auto" } = params;
    ...
    try {
      ...
      if (data.result?.translations?.[0]?.beams) {
        const translations = data.result.translations[0].beams.map(beam =>
          beam.sentences.map(sentence => sentence.text).join(" ")
        );
