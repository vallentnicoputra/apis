...
class HostingCheck {
  ...
  isValid(input) {
    ...
    if (!r.test(cd)) {
      return { valid: false, error: "Invalid domain format. Example: domain.com or https://domain.com" };
    }
    return { valid: true, domain: cd };
  }
  ...
  async check({ domain }) {
    try {
      ...
      const response = await axios.get(
        `${this.api.base}${this.api.endpoint.hosting}/${validation.domain}`,
        {
          headers: {
            ...this.headers,
          ...
          result: {
            error:
              response.status === 404
                ...
                : "Unable to check domain, please try a different website."
          }
        };
      }
...
module.exports = (app) => {
  const hostingCheck = new HostingCheck();

  app.get("/tools/hostingcek", checkApiKeyAndLimit, async (req, res) => {
    const { domain } = req.query;
    const result = await hostingCheck.check({ domain });
