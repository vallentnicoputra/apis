...
class JarakChecker {
  ...
  async hitungJarak(from, to) {
    try {
      ...
      const konsumsiBBM = 12; // km per liter
      const hargaBBM = 10000; // Rupiah
      ...
      return {
        ...
        asal: {
          nama: from,
          koordinat: { lat: fromData.lat, lon: fromData.lon },
        ...
        tujuan: {
          nama: to,
          koordinat: { lat: toData.lat, lon: toData.lon },
...
module.exports = (app) => {
  ...
  app.get("/tools/jarak", checkApiKeyAndLimit, async (req, res) => {
    try {
