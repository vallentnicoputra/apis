...
module.exports = (app) => {
  app.get("/tools/barcoderead", checkApiKeyAndLimit, async (req, res) => {
    const { url } = req.query;
    if (!url) {
      return res.status(400).json({ status: false, error: "Parameter 'url' wajib diisi" });
    }

    const recognizer = new BarcodeRecognizer();
    const result = await recognizer.getDecodedData(url);
