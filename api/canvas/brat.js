...
module.exports = (app) => {
  app.get("/canvas/brat", checkApiKeyAndLimit, async (req, res) => {
    try {
      const { text, fontSize, blur } = req.query;
      ...
      const bratGen = new BratGenerator();
      const imageBuffer = await bratGen.create(text, fontSize, blur);
